...
class TicketAuthorInfoSystem:
    ...
    async def get_user_info(self, guild, user_id, fetch_from_api=True):
        ...
        try:
            ...
            if member:
                logger.info(f"<:j_icons_Correct:1447124105311682673> Direct guild lookup found member {user_id} in server {guild.id}")
            ...
            try:
                ...
                member = guild.get_member(user_id)
                if member:
                    logger.info(f"<:j_icons_Correct:1447124105311682673> Found member {user_id} after chunking guild {guild.id}")
                    return await self._get_member_info(member)
                else:
                    logger.info(f"<:icons_Wrong:1447124112030826516> Member {user_id} definitively not in guild {guild.id} after chunking")
                    
            except Exception as chunk_error:
                logger.warning(f"<:warning:1447124116816793652> Failed to chunk guild {guild.id}: {chunk_error}")
            
            logger.info(f"🔍 Performing manual member search for {user_id} in guild {guild.id}")
            for member in guild.members:
                if member.id == user_id:
                    logger.info(f"<:j_icons_Correct:1447124105311682673> Manual search found member {user_id} in guild {guild.id}")
                    return await self._get_member_info(member)
            
            logger.info(f"<:icons_Wrong:1447124112030826516> User {user_id} confirmed NOT in server {guild.id} - trying API")
            if fetch_from_api:
                try:
                    ...
                    logger.info(f"<:Icons_Trash:1447124221850554458> User {user_id} account deleted")
                    return await self._get_deleted_user_info(user_id)
                except discord.HTTPException as e:
                    logger.warning(f"<:warning:1447124116816793652> HTTP error fetching user {user_id}: {e}")
            ...
            logger.error(f"<:icons_Wrong:1447124112030826516> Error getting user info for {user_id}: {e}")
    ...
    def _create_left_user_embed(self, info):
        ...
        embed.add_field(
            name="Status",
            value="```<:warning:1447124116816793652> User has left the server```",
            inline=False
        )
    ...
    def _create_deleted_embed(self, info):
        ...
        embed.add_field(
            name="Status",
            value="```<:icons_Wrong:1447124112030826516> User account has been deleted```",
            inline=False
        )
...
class UserAvatarView(discord.ui.View):
    ...
    @discord.ui.button(label="View Avatar", style=discord.ButtonStyle.primary, emoji="<:icons_heart:1447124283519271105>")
    async def view_avatar(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            if self.user_info['type'] in ['member', 'left_user']:
                ...
                await interaction.response.send_message("<:icons_Wrong:1447124112030826516> Avatar not available for this user", ephemeral=True)
        except Exception as e:
            logger.error(f"Error displaying avatar: {e}")
            await interaction.response.send_message(f"<:icons_Wrong:1447124112030826516> Error displaying avatar: {str(e)}", ephemeral=True)
...
class TicketClosedLogView(discord.ui.View):
    ...
    @discord.ui.button(label="Ticket Author Info", style=discord.ButtonStyle.secondary, emoji="<:id_icons:1447124382714560705>", custom_id="advanced_ticket_author_info")
    async def author_info(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not creator_id:
                await interaction.followup.send("<:icons_Wrong:1447124112030826516> **Creator ID not found in ticket data**", ephemeral=True)
                return
            ...
            await interaction.followup.send(f"<:icons_Wrong:1447124112030826516> **System Error:** {str(e)}", ephemeral=True)
