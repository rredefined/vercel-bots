        ...
        try:
            if interaction.user.id != self.creator_id:
                await interaction.response.send_message(
                    "<:icons_Wrong:1447124112030826516> Only the ticket creator can rate this ticket.",
                    ephemeral=True
                )
                return
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if existing_rating:
                    await interaction.response.send_message(
                        "<:icons_Wrong:1447124112030826516> You have already submitted a rating for this ticket.",
                        ephemeral=True
                    )
                    return
            ...
            try:
                await interaction.response.send_message(
                    "<:icons_Wrong:1447124112030826516> An error occurred while processing your rating. Please try again.",
                    ephemeral=True
                )
            except:
                pass
