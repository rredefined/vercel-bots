...
def format_priority_emoji(priority: str) -> str:
    priority_emojis = {
        "Low": "<a:green_circle2:1447124249201606777>",
        "Medium": "<:Yellow_circle:1447124254632968192>", 
        "High": "<:icons_fire:1447124305614737450>",
        "Critical": "<:icons_Wrong:1447124112030826516>"
    }
    return priority_emojis.get(priority, "<:Yellow_circle:1447124254632968192>")
...
def get_status_emoji(status: str) -> str:
    status_emojis = {
        "open": "<a:green_circle2:1447124249201606777>",
        "closed": "<:icons_Wrong:1447124112030826516>",
        "locked": "<:icons_locked:1447124150513696830>",
        "claimed": "<:welcome:1447124320286674997>"
    }
    return status_emojis.get(status, "<:icons_help:1447124237176410172>")
...
async def send_transcript_dm(user, channel_name, transcript_file):
    try:
        ...
        transcript_embed = discord.Embed(
            title="<:clipboard1:1447124356130934835> Ticket Transcript",
...
async def send_error_embed(interaction_or_ctx, title: str, description: str):
    embed = discord.Embed(
        title=f"<:icons_Wrong:1447124112030826516> {title}",
...
async def send_success_embed(interaction_or_ctx, title: str, description: str):
    embed = discord.Embed(
        title=f"<:j_icons_Correct:1447124105311682673> {title}",
        description=description,
        color=0x00D4FF
    )
...
async def create_ticket_channel(bot, guild, creator, category, subject, description, ticket_number):
    try:
        ...
        embed = discord.Embed(
            title=f"<:Ticket_icons:1447124189650878464> Ticket #{ticket_number:04d}",
            description=f"**Category:** {category}\n**Subject:** {subject}\n**Description:** {description}",
            color=0x00D4FF
        )
        embed.add_field(name="Creator", value=creator.mention, inline=True)
        embed.add_field(name="Status", value="<a:green_circle2:1447124249201606777> Open", inline=True)
