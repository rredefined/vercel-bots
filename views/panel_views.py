        ...
        try:
            ...
            if not self.values:
                await interaction.response.send_message(
                    "<:icons_Wrong:1447124112030826516> No category selected. Please try again.",
                    ephemeral=True
                )
                return
            ...
            if open_tickets >= ticket_limit:
                await interaction.response.send_message(
                    f"<:Ticket_icons:1447124189650878464> You already have {open_tickets} open tickets. Please close some before creating new ones.",
                    ephemeral=True
                )
                return
            ...
            try:
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        f"<:icons_Wrong:1447124112030826516> Error: {type(e).__name__}: {str(e)[:150]}",
                        ephemeral=True
                    )
                else:
