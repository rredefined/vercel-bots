...
class TicketControlView(discord.ui.View):
    ...
    @discord.ui.button(label="Close Ticket", style=discord.ButtonStyle.danger, emoji="<:welcome:1447124320286674997>", custom_id="close_ticket_btn", row=0)
        ...
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if not result:
                    embed = discord.Embed(
                        ...
                        description="**The ticket system is not properly configured.**\n\n"
                                   "<:icons_wrench:1447124178296766494> **Issue:** Missing system configuration\n"
                                   "<:lightbulb:1447124133107204166> **Solution:** Contact an administrator to resolve this issue\n"
                ...
                if not (is_creator or is_support or is_admin):
                    embed = discord.Embed(
                        title="<:icons_locked:1447124150513696830> Access Restricted",
                    ...
                    embed.add_field(
                        name="<:Target:1447124315211567134> Authorized Users",
                        value=f"**<:icons_Person:1447124205563805698> Ticket Creator:** Original requester\n"
                              f"**<:shield:1447124194528858143> Support Staff:** {support_role.mention if support_role else 'Support Role'}\n"
                              f"**<:LM_Icons_Crown:1447139233373487154> Administrators:** Server administrators",
                        inline=False
                    )

                    embed.add_field(
                        name="<:lightbulb:1447124133107204166> Need Help?",
            ...
            confirmation_embed = discord.Embed(
                title="<:icons_locked:1447124150513696830> Ticket Closure Confirmation",
                description=f"**Are you sure you want to close this support ticket?**\n\n"
                           f"<:warning:1447124116816793652> **Important:** This action is permanent and cannot be undone.\n\n"
                           f"**What happens when you close:**\n"
                           f"• <:clipboard1:1447124356130934835> Complete conversation transcript will be generated\n"
                           f"• <:icons_email:1447139227644203111> Transcript sent to ticket creator's DMs\n"
                           f"• <:icons_folder:1447124215529603134> Ticket logged in support system\n"
                           f"• <:type_icons:1447139212896764037> Channel will be permanently deleted\n"
                           f"• <:icons_star:1447124294545965109> Rating request sent to customer",
                color=0xFF6B6B,
                timestamp=discord.utils.utcnow()
            )

            confirmation_embed.add_field(
                name="<:Ticket_icons:1447124189650878464> Ticket Information",
    ...
    @discord.ui.button(label="Claim Ticket", style=discord.ButtonStyle.success, emoji="<:bye:1447124138798874674>", custom_id="claim_ticket_btn", row=0)
    async def claim_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if not result:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> System Error",
                ...
                if not has_support_access:
                    embed = discord.Embed(
                        title="<:shield:1447124194528858143> Access Restricted",
                        description="**Only support staff members can claim tickets.**\n\n"
                                   ...
                                   f"<:clipboard1:1447124356130934835> **Your Roles:** {', '.join([role.mention for role in interaction.user.roles if role != interaction.guild.default_role][:3])}\n\n"
                ...
                if claim_result and claim_result[0]:
                    ...
                    if claim_result[0] == interaction.user.id:
                        embed = discord.Embed(
                            ...
                            description=f"**You have already claimed this ticket.**\n\n"
                                       ...
                                       f"<:clipboard1:1447124356130934835> **Action:** No further action needed\n"
                                       f"<:type_icons:1447139212896764037> **Continue:** Assist the customer as normal",
                            color=0x00FF88
                        )
                        await interaction.response.send_message(embed=embed, ephemeral=True)
                        return
                    ...
                    embed = discord.Embed(
                        title="<:icons_locked:1447124150513696830> Already Claimed",
                        description=f"**This ticket has already been claimed and is being handled.**\n\n"
                                   f"<:Target:1447124315211567134> **Assigned Agent:** {claimer.mention if claimer else 'Unknown Agent'}\n"
                                   f"<:label:1447139238717034607> **Agent Name:** {claimer.display_name if claimer else 'Unknown'}\n"
                                   f"<:clipboard1:1447124356130934835> **Status:** 🟢 Active Support\n\n"
                                   f"<:lightbulb:1447124133107204166> **Need to reassign?** Use `/transfer-ticket @new_agent` command",
                        color=0xFF8C00
                    )
                    await interaction.response.send_message(embed=embed, ephemeral=True)
                    return
...
class SetupWizardView(discord.ui.View):
    ...
    async def start_setup(self, interaction):
        ...
        embed.set_footer(text="RenderByte™ • Setup Wizard • Step 1 of 1")
    ...
    async def handle_custom_message(self, message):
        ...
        if self.waiting_for_custom == "role":
            if message.role_mentions:
                ...
                await message.reply(f"<:j_icons_Correct:1447124105311682673> Custom support role set to {role.mention}")
            else:
                await message.reply("<:icons_Wrong:1447124112030826516> Please mention a valid role!")
        elif self.waiting_for_custom == "panel_channel":
            if message.channel_mentions:
                channel = message.channel_mentions[0]
                self.setup_data['channel_id'] = channel.id
                await message.reply(f"<:j_icons_Correct:1447124105311682673> Custom panel channel set to {channel.mention}")
            else:
                await message.reply("<:icons_Wrong:1447124112030826516> Please mention a valid channel!")
        elif self.waiting_for_custom == "log_channel":
            if message.channel_mentions:
                channel = message.channel_mentions[0]
                self.setup_data['log_channel_id'] = channel.id
                await message.reply(f"<:j_icons_Correct:1447124105311682673> Custom log channel set to {channel.mention}")
            else:
                await message.reply("<:icons_Wrong:1447124112030826516> Please mention a valid channel!")
    ...
    async def finish_setup(self):
        try:
            ...
            return True, "<:j_icons_Correct:1447124105311682673> Setup completed successfully!"

        except Exception as e:
            logger.error(f"Error finishing setup: {e}")
            return False, f"<:icons_Wrong:1447124112030826516> Setup failed: {str(e)}"

class PanelChannelSelect(discord.ui.ChannelSelect):
    def __init__(self):
        super().__init__(
            placeholder="<:Ticket_icons:1447124189650878464> Select Ticket Panel Channel...",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="<:j_icons_Correct:1447124105311682673> Panel Channel Selected",
...
class LogChannelSelect(discord.ui.ChannelSelect):
    def __init__(self):
        super().__init__(
            placeholder="<:clipboard1:1447124356130934835> Select Logs Channel...",
            channel_types=[discord.ChannelType.text],
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="<:j_icons_Correct:1447124105311682673> Log Channel Selected", 
...
class SupportRoleSelect(discord.ui.RoleSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="<:j_icons_Correct:1447124105311682673> Support Role Selected",
...
class CustomRoleButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            emoji="<:shield:1447124194528858143>",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="<:shield:1447124194528858143> Custom Support Role",
...
class CustomPanelChannelButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            style=discord.ButtonStyle.secondary,
            emoji="<:megaphone:1447124271901184051>",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="<:megaphone:1447124271901184051> Custom Panel Channel",
...
class CustomLogChannelButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            style=discord.ButtonStyle.secondary,
            emoji="<:stats_1:1447124183686320259>",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="<:stats_1:1447124183686320259> Custom Log Channel",
...
class PanelCustomizationButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            emoji="<:paint_icons:1447124340096368762>",
...
class ConfirmSetupButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            ...
            emoji="<:j_icons_Correct:1447124105311682673>",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not view.setup_data['channel_id']:
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Missing Panel Channel",
        ...
        if not view.setup_data['role_id']:
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Missing Support Role",
        ...
        embed = discord.Embed(
            title="<:j_icons_Correct:1447124105311682673> Setup Configuration Preview",
...
class FinalConfirmView(discord.ui.View):
    ...
    @discord.ui.button(label="Finish Setup", style=discord.ButtonStyle.success, emoji="<:UA_Rocket_icons:1447124128342478990>")
    async def finish_setup(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if success:
            embed = discord.Embed(
                title="<:giveaway_icons:1447124371939262465> Setup Complete!",
                description="**Your ticket system is ready!**\n\n"
                           ...
                           "**<:glowingstar:1447139206202658876> Pro Tips:**\n"
            ...
            embed.set_footer(text="RenderByte™ • Support System Active")
        else:
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Setup Failed",
    ...
    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.danger, emoji="<:icons_Wrong:1447124112030826516>")
    async def cancel_setup(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            title="<:icons_Wrong:1447124112030826516> Setup Cancelled",
...
class TicketSetupView(discord.ui.View):
    def __init__(self, bot, ctx):
        ...
        self.setup_data = {
            ...
            'embed_color': 0x2F3136,
            'embed_image_url': None,
            'embed_footer': 'Powered by RemderNuye™ • Support System',
            'ticket_limit': 3
        }
        self.waiting_for_custom = None
...
class SetupPanelChannelSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "custom_channel":
            ...
            embed = discord.Embed(
                title="<:Ticket_icons:1447124189650878464> Panel Channel Selected",
...
class SetupLogChannelSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "custom_log_channel":
            ...
            embed = discord.Embed(
                title="<:clipboard1:1447124356130934835> Log Channel Selected", 
...
class SetupSupportRoleSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "custom_role":
            ...
            embed = discord.Embed(
                title="<:people_icons:1447124377551507629> Support Role Selected",
...
class NewPanelCustomizationModal(discord.ui.Modal):
    ...
    panel_footer = discord.ui.TextInput(
        ...
        default="Powered by RenderByte™ • Support System",
        max_length=100,
        required=False
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Panel Customization Saved",
            ...
            error_embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Customization Error",
...
class TicketChannelView(discord.ui.View):
    ...
    def add_close_button(self):
        ...
        close_button = discord.ui.Button(
            ...
            emoji="<:welcome:1447124320286674997>",
...
class TicketCloseConfirmationView(discord.ui.View):
    ...
    @discord.ui.button(label="Continue", style=discord.ButtonStyle.danger, emoji="<:j_icons_Correct:1447124105311682673>")
    async def confirm_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if creator:
                try:
                    closure_embed = discord.Embed(
                        title="<:icons_locked:1447124150513696830> Your Ticket Has Been Closed",
                    ...
                    try:
                        fallback_embed = discord.Embed(
                            title="<:icons_Wrong:1447124112030826516> DM Error",
            ...
            await interaction.followup.send("<:j_icons_Correct:1447124105311682673> Ticket closed successfully.", ephemeral=True)
            await asyncio.sleep(1)
            await channel.delete(reason=f"Ticket #{ticket_number:04d} closed by {interaction.user}")

        except Exception as e:
            logger.error(f"Error closing ticket: {e}")
            await interaction.followup.send(f"<:icons_Wrong:1447124112030826516> Error closing ticket: {str(e)}", ephemeral=True)

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary, emoji="<:icons_Wrong:1447124112030826516>")
    async def cancel_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("<:icons_Wrong:1447124112030826516> Ticket close cancelled.", ephemeral=True)
