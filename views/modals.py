        ...
        try:
            ...
            if await check_rate_limit(self.bot, user_id):
                await interaction.followup.send(
                    "<:icons_Wrong:1447124112030826516> You're creating tickets too quickly. Please wait 60 seconds before creating another ticket.",
                    ephemeral=True
                )
                return
            ...
            if not ticket_limit_ok:
                await interaction.followup.send(
                    f"<:icons_Wrong:1447124112030826516> You have reached the maximum ticket limit ({current_tickets}/{max_tickets}). Please close existing tickets before creating new ones.",
                    ephemeral=True
                )
                return
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if await cur.fetchone():
                    await interaction.followup.send(
                        "<:icons_Wrong:1447124112030826516> You are blacklisted from creating tickets in this server.",
                        ephemeral=True
                    )
                    return
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if result and result[0]:
                    await interaction.followup.send(
                        "<:icons_wrench:1447124178296766494> The ticket system is currently under maintenance. Please try again later.",
                        ephemeral=True
                    )
                    return
            ...
            try:
                ...
                if success:
                    ...
                    await interaction.followup.send(
                        f"<:j_icons_Correct:1447124105311682673> {message}",
                        ephemeral=True
                    )
                else:
                    await interaction.followup.send(
                        f"<:icons_Wrong:1447124112030826516> {message}",
                        ephemeral=True
                    )
            except Exception as ticket_error:
                logger.error(f"Error creating ticket: {ticket_error}")
                await interaction.followup.send(
                    f"<:icons_Wrong:1447124112030826516> Failed to create ticket: {str(ticket_error)}",
                    ephemeral=True
                )
            ...
            try:
                await interaction.followup.send(
                    "<:icons_Wrong:1447124112030826516> An error occurred while creating your ticket. Please try again.",
                    ephemeral=True
                )
            except:
                pass
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception):
        ...
        try:
            await interaction.response.send_message(
                "<:icons_Wrong:1447124112030826516> An error occurred. Please try again.",
                ephemeral=True
            )
        except:
            pass
...
class PanelCustomizationModal(discord.ui.Modal):
    def __init__(self, setup_view):
        super().__init__(title="<:paint_icons:1447124340096368762> Panel Customization")
        self.setup_view = setup_view

    embed_title = discord.ui.TextInput(
        label="Panel Title",
        placeholder="e.g., <:Ticket_icons:1447124189650878464> Support Center",
        default="<:Ticket_icons:1447124189650878464> Support Center",
        max_length=100,
        required=True
    )
    ...
    embed_description = discord.ui.TextInput(
        ...
        style=discord.TextStyle.paragraph,
        default="Need assistance? Select a category below to create a support ticket. Our expert team will help you shortly! <:UA_Rocket_icons:1447124128342478990>",
        max_length=500,
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed.add_field(
                name="<:paint_icons:1447124340096368762> Customization Preview",
...
class TicketSetupModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not channel:
                await interaction.followup.send("<:icons_Wrong:1447124112030826516> Support channel not found!", ephemeral=True)
                return

            if not role:
                await interaction.followup.send("<:icons_Wrong:1447124112030826516> Support role not found!", ephemeral=True)
                return
            ...
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Setup Complete",
            ...
            await interaction.followup.send("<:icons_Wrong:1447124112030826516> Invalid ID format! Please use numbers only.", ephemeral=True)
        except Exception as e:
            logger.error(f"Error in setup modal: {e}")
            await interaction.followup.send(f"<:icons_Wrong:1447124112030826516> Setup failed: {str(e)}", ephemeral=True)
...
class TicketLimitModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Invalid Input",
