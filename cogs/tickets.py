...
class SupportSystem(commands.Cog):
            ...
            async with self.bot.db.cursor() as cur:
                await cur.execute("""
                    CREATE TABLE IF NOT EXISTS tickets (
                        ...
                        embed_title TEXT DEFAULT '<:Ticket_icons:1447124189650878464> Support Center',
                        embed_description TEXT DEFAULT 'Need help? Select a category below to create a support ticket. Our team will assist you shortly! <:UA_Rocket_icons:1447124128342478990>',
    ...
    async def setup_tickets(self, ctx: commands.Context):
        ...
        try:
            ...
            if not await check_database_connection(self.bot):
                message = "<:icons_Wrong:1447124112030826516> | Database connection failed. Please try again later."
                if isinstance(ctx, discord.Interaction):
            ...
            embed = discord.Embed(
                title="<:UA_Rocket_icons:1447124128342478990>  Support Setup",
                description=f"**Welcome to the Advanced Ticket System Setup!**\n\n"
                            ...
                            f"**<:clipboard1:1447124356130934835> Required Steps:**\n"
                            ...
                            f"<:icons_clock:1447124144348201014> **Setup expires in 29 minutes**",
                color=0x00D4FF,
                timestamp=current_time
            )
            embed.add_field(
                name="<:lightbulb:1447124133107204166> Pro Tips",
            ...
            error_message = f"<:icons_Wrong:1447124112030826516> | An error occurred: {e}"
            if isinstance(ctx, discord.Interaction):
    ...
    async def add_category(self, ctx: commands.Context, category: str, emoji: str = None):
        ...
        try:
            ...
            if len(category) > 25:
                message = "<:icons_Wrong:1447124112030826516> | Category name cannot exceed 25 characters."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message, ephemeral=True)
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Success" if success else "<:icons_Wrong:1447124112030826516> Error",
                description=message,
            ...
            if success:
                ...
                if not panel_success and "not set up" not in panel_message:
                    error_message = f"<:icons_Wrong:1447124112030826516> | Failed to update support panel: {panel_message}"
                    if isinstance(ctx, discord.Interaction):
                        await ctx.followup.send(error_message, ephemeral=True)
                    else:
                        await ctx.send(error_message, ephemeral=True)
    ...
    async def list_categories(self, ctx: commands.Context):
        ...
        try:
            ...
            if not (user_has_support or is_admin):
                embed = discord.Embed(
                    title="<:icons_locked:1447124150513696830> Permission Denied",
                ...
                embed.add_field(
                    name="<:Target:1447124315211567134> Required Permission",
            ...
            if not categories:
                message = "<:icons_Wrong:1447124112030826516> | No support categories found. Use `/add-category <category>` to add some."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
            ...
            for category_name, emoji in categories:
                display_emoji = emoji if emoji else "<:Ticket_icons:1447124189650878464>"
                category_list.append(f"• {display_emoji} {category_name}")
            
            embed = discord.Embed(
                title="<:clipboard1:1447124356130934835> Support Categories",
                description="\n".join(category_list),
                color=0x00D4FF,
                timestamp=current_time
            )
    ...
    async def send_panel(self, ctx: commands.Context, type: str):
        ...
        try:
            ...
            if not categories:
                current_time = utc_to_gmt(discord.utils.utcnow())
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> No Categories Found",
                    description="You must create at least one ticket category before sending a panel.\n\n"
                              "**<:lightbulb:1447124133107204166> How to add categories:**\n"
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Support Panel Sent" if success else "<:icons_Wrong:1447124112030826516> Error",
                description=message,
                color=0x00D4FF if success else 0xFF6B6B,
                timestamp=current_time
            )
    ...
    async def close_ticket(self, ctx: commands.Context):
        ...
        try:
            ...
            if not await is_ticket_channel(self.bot, ctx.channel):
                message = "<:icons_Wrong:1447124112030826516> | This command can only be used in a support ticket channel."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if not ticket_role:
                message = "<:icons_Wrong:1447124112030826516> | Support system is not set up properly."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if not ticket_creator_id:
                message = "<:icons_Wrong:1447124112030826516> | Could not determine the ticket creator."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if not (invoker == ticket_creator or user_has_support):
                message = "<:icons_Wrong:1447124112030826516> | You do not have permission to close this ticket."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if not ticket_info:
                message = "<:icons_Wrong:1447124112030826516> | Could not retrieve ticket information."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message, ephemeral=True)
            ...
            confirmation_embed = discord.Embed(
                title="<:icons_locked:1447124150513696830> Close Ticket Confirmation",
            ...
            error_message = f"<:icons_Wrong:1447124112030826516> | I don't have permission to delete the channel: {e}"
            if isinstance(ctx, discord.Interaction):
                await ctx.followup.send(error_message, ephemeral=True)
            else:
                await ctx.send(error_message)
    ...
    async def set_limit(self, ctx: commands.Context, limit: int):
        ...
        try:
            ...
            if limit < 1 or limit > 10:
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Invalid Limit",
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if cur.rowcount == 0:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Setup Required",
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Ticket Limit Updated",
    ...
    async def reset_categories(self, ctx: commands.Context):
        ...
        try:
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:Icons_Trash:1447124221850554458> Categories Reset" if success else "<:icons_Wrong:1447124112030826516> Error",
                description=message,
                color=0x00D4FF if success else 0xFF6B6B,
                timestamp=current_time
            )
    ...
    async def transfer_ticket(self, ctx: commands.Context, member: discord.Member):
        ...
        try:
            ...
            if not user_has_support:
                message = "<:icons_Wrong:1447124112030826516> | You do not have permission to transfer tickets."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if not target_has_support:
                message = f"<:icons_Wrong:1447124112030826516> | {member.mention} does not have the support role."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:icons_Person:1447124205563805698> Ticket Transferred",
            ...
            success_message = f"<:j_icons_Correct:1447124105311682673> | Ticket successfully transferred to {member.mention}."
            if isinstance(ctx, discord.Interaction):
    ...
    async def add_user(self, ctx: commands.Context, user: discord.Member):
        ...
        try:
            ...
            if not user_has_support:
                message = "<:icons_Wrong:1447124112030826516> | Only support staff can add users to tickets."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:welcome:1447124320286674997> User Added to Ticket",
            ...
            success_message = f"<:j_icons_Correct:1447124105311682673> | {user.mention} has been added to the ticket."
            if isinstance(ctx, discord.Interaction):
                await ctx.followup.send(success_message, ephemeral=True)
            else:
                await ctx.send(success_message, ephemeral=True)
    ...
    async def remove_user(self, ctx: commands.Context, user: discord.Member):
        ...
        try:
            ...
            if not user_has_support:
                message = "<:icons_Wrong:1447124112030826516> | Only support staff can remove users from tickets."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if user.id == ticket_creator_id:
                message = "<:icons_Wrong:1447124112030826516> | You cannot remove the ticket creator from their own ticket."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            if ticket_role and ticket_role in user.roles:
                message = "<:icons_Wrong:1447124112030826516> | You cannot remove support staff from tickets."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(message, ephemeral=True)
                else:
                    await ctx.send(message)
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> User Removed from Ticket",
            ...
            success_message = f"<:j_icons_Correct:1447124105311682673> | {user.mention} has been removed from the ticket."
            if isinstance(ctx, discord.Interaction):
                await ctx.followup.send(success_message, ephemeral=True)
            else:
                await ctx.send(success_message, ephemeral=True)
    ...
    async def ticket_info(self, ctx: commands.Context):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"<:clipboard1:1447124356130934835> Ticket Information",
            ...
            embed.add_field(
                name="<:Ticket_icons:1447124189650878464> Ticket Details",
                value=f"**Number:** #{ticket_info['ticket_number']:04d}\n"
                      f"**Status:** <:j_icons_Correct:1447124105311682673> Open\n"
                      f"**Messages:** {message_count}",
                inline=True
            )

            embed.add_field(
                name="<:icons_Person:1447124205563805698> Creator Information",
            ...
            embed.add_field(
                name="<:Icons_calender:1447124211012341811> Timeline",
            ...
            embed.add_field(
                name="<:welcome:1447124320286674997> Access List",
    ...
    async def rename_ticket(self, ctx: commands.Context, *, name: str):
        ...
        try:
            ...
            if not await is_ticket_channel(self.bot, ctx.channel):
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Not a Ticket Channel",
                    description="**This command can only be used in support ticket channels.**\n\n"
                               "<:Ticket_icons:1447124189650878464> This command is designed for ticket management and can only be used within active support tickets.",
                    color=0xFF6B6B
                )
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(embed=embed, ephemeral=True)
            ...
            if len(name) > 100:
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Name Too Long",
            ...
            if len(name.strip()) == 0:
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Invalid Name",
            ...
            if not sanitized_name:
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Invalid Characters",
                    description="**The name contains only invalid characters.**\n\n"
                               "<:clipboard1:1447124356130934835> **Valid characters:**\n"
                               "• Letters (a-z)\n"
                               "• Numbers (0-9)\n"
                               "• Dashes (-)\n"
                               "• Underscores (_)\n\n"
            ...
            if old_name == sanitized_name:
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Same Name",
            ...
            if name_changed:
                preview_embed = discord.Embed(
                    title="<:clipboard1:1447124356130934835> Name Preview",
                ...
                preview_embed.add_field(
                    name="<:j_icons_Correct:1447124105311682673> Auto-Adjustments Made",
            ...
            try:
                priority_emoji = ctx.channel.name[0] if ctx.channel.name else "🟡"
                valid_emojis = ["🟢", "🟡", "🟠", "🔴"]
                if priority_emoji not in valid_emojis:
                    priority_emoji = "🟡"

                await ctx.channel.edit(
                    name=f"{priority_emoji}{sanitized_name}",
                ...
                if "name" in error_msg or "invalid" in error_msg:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Invalid Channel Name",
            ...
            success_embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Ticket Renamed Successfully",
            ...
            try:
                ...
                if log_channel:
                    ...
                    log_embed = discord.Embed(
                        title="<:clipboard1:1447124356130934835> Ticket Renamed",
            ...
            confirmation_message = f"<:j_icons_Correct:1447124105311682673> **Ticket successfully renamed to `{sanitized_name}`**"
            if isinstance(ctx, discord.Interaction):
            ...
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Permission Error",
                description="**I don't have permission to rename this channel.**\n\n"
                           "<:icons_wrench:1447124178296766494> **Required Bot Permissions:**\n"
                           "• Manage Channels\n"
                           "• View Channel\n\n"
            ...
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Discord API Error",
            ...
            embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Unexpected Error",
        ...
        try:
            ...
            if not await is_ticket_channel(self.bot, ctx.channel):
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Not a Ticket Channel",
                    description="**This command can only be used in support ticket channels.**\n\n"
                               "<:Ticket_icons:1447124189650878464> Use this command within an active support ticket to claim it for handling.",
                    color=0xFF6B6B
                )
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(embed=embed, ephemeral=True)
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if not result:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> System Configuration Error",
                ...
                if not (has_support_role or is_admin):
                    embed = discord.Embed(
                        title="<:shield:1447124194528858143> Permission Denied",
                        description="**Only support staff can claim tickets.**\n\n"
                                   f"<:Target:1447124315211567134> **Required Role:** {support_role.mention if support_role else 'Support Role'}\n"
                                   f"<:icons_Person:1447124205563805698> **Your Roles:** {', '.join([role.mention for role in invoker.roles if role != ctx.guild.default_role][:3]) or 'No special roles'}\n\n"
                                   f"<:lightbulb:1447124133107204166> **Need access?** Contact an administrator to get the support role.",
                        color=0xFF6B6B
                    )
                    if isinstance(ctx, discord.Interaction):
                        await ctx.followup.send(embed=embed, ephemeral=True)
                ...
                if not ticket_result:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Ticket Not Found",
                ...
                if current_claimer_id:
                    if current_claimer_id == invoker.id:
                        embed = discord.Embed(
                            title="<:j_icons_Correct:1447124105311682673> Already Your Ticket",
                            description=f"**You have already claimed this ticket.**\n\n"
                                       f"<:Target:1447124315211567134> **Status:** This ticket is assigned to you\n"
                                       f"<:clipboard1:1447124356130934835> **Action:** Continue providing support\n"
                                       f"<:type_icons:1447139212896764037> **Next Step:** Assist the customer as normal",
                            color=0x00FF88
                        )
                        if isinstance(ctx, discord.Interaction):
                            await ctx.followup.send(embed=embed, ephemeral=True)
                    ...
                    embed = discord.Embed(
                        title="<:icons_locked:1447124150513696830> Ticket Already Claimed",
                        description=f"**This ticket is already being handled by another agent.**\n\n"
                                   f"<:Target:1447124315211567134> **Current Agent:** {claimer.mention if claimer else f'<@{current_claimer_id}>'}\n"
                                   f"<:label:1447139238717034607> **Agent Name:** {claimer.display_name if claimer else 'Unknown Agent'}\n"
                                   f"<:clipboard1:1447124356130934835> **Status:** 🟢 Currently Active\n\n"
                                   f"<:lightbulb:1447124133107204166> **Need to transfer?** Use `/transfer-ticket @new_agent` command",
                        color=0xFF8C00
                    )
                    if isinstance(ctx, discord.Interaction):
                        await ctx.followup.send(embed=embed, ephemeral=True)
            ...
            confirmation_msg = f"<:j_icons_Correct:1447124105311682673> You have successfully claimed ticket #{ticket_number:04d}!"
            if isinstance(ctx, discord.Interaction):
            ...
            error_embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Claim Error",
        ...
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if await cur.fetchone():
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Already Blacklisted",
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:icons_locked:1447124150513696830> User Blacklisted",
                description=f"**{user.mention} has been blacklisted from creating tickets.**\n\n"
                           f"<:Target:1447124315211567134> **User:** {user.display_name} (`{user.id}`)\n"
                           f"<:icons_Person:1447124205563805698> **Blacklisted by:** {ctx.author.mention if isinstance(ctx, commands.Context) else ctx.user.mention}\n"
                           f"<:Icons_calender:1447124211012341811> **Date:** {current_time.strftime('%B %d, %Y at %I:%M %p GMT')}",
                color=0xFF6B6B,
                timestamp=current_time
            )
            embed.add_field(
                name="<:lightbulb:1447124133107204166> Effect",
    ...
    async def blacklist_remove_user(self, ctx: commands.Context, user: discord.Member):
        ...
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if not result:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> User Not Blacklisted",
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> User Removed from Blacklist",
                description=f"**{user.mention} has been removed from the ticket blacklist.**\n\n"
                           f"<:Target:1447124315211567134> **User:** {user.display_name} (`{user.id}`)\n"
                           f"<:icons_Person:1447124205563805698> **Removed by:** {ctx.author.mention if isinstance(ctx, commands.Context) else ctx.user.mention}\n"
                           f"<:Icons_calender:1447124211012341811> **Removed on:** {current_time.strftime('%B %d, %Y at %I:%M %p GMT')}",
                color=0x00FF88,
                timestamp=current_time
            )
            embed.add_field(
                name="<:j_icons_Correct:1447124105311682673> Effect",
    ...
    async def blacklist_list(self, ctx: commands.Context):
        ...
        try:
            ...
            if not blacklisted:
                embed = discord.Embed(
                    title="<:j_icons_Correct:1447124105311682673> No Blacklisted Users",
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:icons_locked:1447124150513696830> Blacklisted Users",
            ...
            embed.add_field(
                name="<:clipboard1:1447124356130934835> Blacklisted Users",
            ...
            if len(blacklisted) > 10:
                embed.add_field(
                    name="<:lightbulb:1447124133107204166> Note",
    ...
    async def faq(self, ctx: commands.Context):
        ...
        try:
            ...
            current_time = utc_to_gmt(discord.utils.utcnow())
            embed = discord.Embed(
                title="<:icons_help:1447124237176410172> Frequently Asked Questions",
            ...
            embed.add_field(
                name="<:icons_help:1447124237176410172> **Available Categories**",
                value="**<:Ticket_icons:1447124189650878464> Getting Started** - Creating tickets, basics\n"
                      "**<:icons_clock:1447124144348201014> Response & Priority** - Response times, urgency\n"
                      "**<:Target:1447124315211567134> Ticket Management** - Managing, closing, users\n"
                      "**<:clipboard1:1447124356130934835> Features & Settings** - Transcripts, limits, ratings\n"
                      "**<:icons_wrench:1447124178296766494> Troubleshooting** - Common issues, solutions",
                inline=False
            )

            embed.add_field(
                name="<:UA_Rocket_icons:1447124128342478990> **Quick Access**",
    ...
    async def maintenance_mode(self, ctx: commands.Context):
        ...
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if not result:
                    message = "<:icons_Wrong:1447124112030826516> | Support system is not set up. Use `/setup-tickets` first."
                    if isinstance(ctx, discord.Interaction):
                        await ctx.followup.send(message, ephemeral=True)
                    else:
                        await ctx.send(message, ephemeral=True)
            ...
            emoji = "<:icons_wrench:1447124178296766494>" if new_mode else "<:j_icons_Correct:1447124105311682673>"

            embed = discord.Embed(
                title=f"{emoji} Maintenance Mode {status}",
                description=f"**Maintenance mode has been {status.lower()}.**\n\n"
                           f"{'<:warning:1447124116816793652> **Ticket creation is now disabled.** Users cannot create new tickets.' if new_mode else '<:j_icons_Correct:1447124105311682673> **Ticket creation is now enabled.** Users can create tickets normally.'}\n\n"
                           f"**Status:** {'<:icons_wrench:1447124178296766494> Under Maintenance' if new_mode else '🟢 Operational'}\n"
    ...
    async def announce(self, ctx: commands.Context, *, message: str):
        ...
        try:
            ...
            if len(message) > 2000:
                error_message = "<:icons_Wrong:1447124112030826516> | Announcement message cannot exceed 2000 characters."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(error_message, ephemeral=True)
                else:
                    await ctx.send(error_message, ephemeral=True)
            ...
            if not tickets:
                message_text = "<:megaphone:1447124271901184051> | No open tickets found to send announcements to."
                if isinstance(ctx, discord.Interaction):
            ...
            announcement_embed = discord.Embed(
                title="<:megaphone:1447124271901184051> System Announcement",
                description=f"**Official announcement from {ctx.guild.name} support team:**\n\n{message}",
                color=0xFF8C00,
                timestamp=current_time
            )
            announcement_embed.add_field(
                name="<:clipboard1:1447124356130934835> Announcement Details",
            ...
            result_embed = discord.Embed(
                title="<:megaphone:1447124271901184051> Announcement Sent",
                description=f"**Your announcement has been delivered to open tickets.**\n\n"
                           f"**<:stats_1:1447124183686320259> Delivery Summary:**\n"
                           f"<:j_icons_Correct:1447124105311682673> **Successfully sent:** {success_count} tickets\n"
                           f"<:icons_Wrong:1447124112030826516> **Failed to send:** {failed_count} tickets\n"
                           f"<:clipboard1:1447124356130934835> **Total tickets:** {len(tickets)} tickets\n\n"
                           f"**<:clipboard1:1447124356130934835> Message Preview:**\n*{message[:100]}{'...' if len(message) > 100 else ''}*",
                color=0x00D4FF,
                timestamp=current_time
            )
        ...
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if primary_role and primary_role[0] == role.id:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Cannot Add Primary Role",
                ...
                if await cur.fetchone():
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Role Already Added",
            ...
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Support Role Added" if success else "<:icons_Wrong:1447124112030826516> Error",
            ...
            if success:
                embed.add_field(
                    name="<:Target:1447124315211567134> Permissions Granted",
    ...
    async def support_role_remove(self, ctx: commands.Context, role: discord.Role):
        ...
        try:
            ...
            async with self.bot.db.cursor() as cur:
                ...
                if primary_role and primary_role[0] == role.id:
                    embed = discord.Embed(
                        title="<:icons_Wrong:1447124112030826516> Cannot Remove Primary Role",
            ...
            embed = discord.Embed(
                title="<:j_icons_Correct:1447124105311682673> Support Role Removed" if success else "<:icons_Wrong:1447124112030826516> Error",
    ...
    async def support_role_list(self, ctx: commands.Context):
        ...
        try:
            ...
            embed = discord.Embed(
                title="<:people_icons:1447124377551507629> Support Roles",
            ...
            embed.add_field(
                name="<:shield:1447124194528858143> Primary Support Role",
            ...
            if additional_roles:
                ...
                embed.add_field(
                    name="<:Target:1447124315211567134> Additional Support Roles",
            ...
            embed.add_field(
                name="<:stats_1:1447124183686320259> Summary",
        ...
        try:
            ...
            if not await is_ticket_channel(self.bot, ctx.channel):
                embed = discord.Embed(
                    title="<:icons_Wrong:1447124112030826516> Not a Ticket Channel",
                    description="**This command can only be used in support ticket channels.**\n\n"
                               "<:Ticket_icons:1447124189650878464> Use this command within an active support ticket to change its priority.",
                    color=0xFF6B6B
                )
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(embed=embed, ephemeral=True)
            ...
            embed = discord.Embed(
                title=f"<:j_icons_Correct:1447124105311682673> Priority Updated",
            ...
            error_embed = discord.Embed(
                title="<:icons_Wrong:1447124112030826516> Error",
    ...
    async def remind(self, ctx: commands.Context, time: str, *, message: Optional[str] = None):
        ...
        try:
            ...
            is_ticket_channell = await is_ticket_channel(self.bot, ctx.channel)

            if not (user_has_support or is_ticket_channell):
                error_message = "<:icons_Wrong:1447124112030826516> | You can only set reminders in ticket channels or if you have the support role."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(error_message, ephemeral=True)
                else:
                    await ctx.send(error_message, ephemeral=True)
            ...
            if not match:
                error_message = "<:icons_Wrong:1447124112030826516> | Invalid time format. Use: 5m (minutes), 1h (hours), 2d (days)"
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(error_message, ephemeral=True)
                else:
                    await ctx.send(error_message, ephemeral=True)
            ...
            if delay_seconds < 60:  # Minimum 1 minute
                error_message = "<:icons_Wrong:1447124112030826516> | Reminder time must be at least 1 minute."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(error_message, ephemeral=True)
                else:
                    await ctx.send(error_message, ephemeral=True)
            ...
            if delay_seconds > 604800:  # Maximum 7 days
                error_message = "<:icons_Wrong:1447124112030826516> | Reminder time cannot exceed 7 days."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(error_message, ephemeral=True)
                else:
                    await ctx.send(error_message, ephemeral=True)
            ...
            if not message:
                message = "<:icons_clock:1447124144348201014> **Reminder:** This is your scheduled follow-up reminder."
            ...
            embed = discord.Embed(
                title="<:icons_clock:1447124144348201014> Advanced Reminder Set",
                description=f"**Your reminder has been scheduled successfully!**\n\n"
                           f"<:Icons_calender:1447124211012341811> **Reminder Details:**\n"
                           f"<:icons_clock:1447124144348201014> **Trigger Time:** {discord.utils.format_dt(remind_time, 'F')}\n"
                           f"📍 **In:** {amount} {time_units[unit]}\n"
                           f"📍 **Relative:** {discord.utils.format_dt(remind_time, 'R')}\n"
                           f"<:clipboard1:1447124356130934835> **Channel:** {ctx.channel.mention}\n"
                           f"<:icons_Person:1447124205563805698> **Set by:** {ctx.author.mention if isinstance(ctx, commands.Context) else ctx.user.mention}\n\n"
                           f"<:type_icons:1447139212896764037> **Reminder Message:**\n*{message}*",
                color=0x00D4FF,
                timestamp=current_time
            )
            ...
            reminder_embed = discord.Embed(
                title="<:icons_clock:1447124144348201014> Scheduled Reminder",
                description=f"**This is your scheduled reminder!**\n\n"
                           f"<:clipboard1:1447124356130934835> **Message:** {message}\n\n"
                           f"<:clipboard1:1447124356130934835> **Reminder Details:**\n"
                           f"<:icons_Person:1447124205563805698> **Set by:** {ctx.author.mention if isinstance(ctx, commands.Context) else ctx.user.mention}\n"
                           f"<:icons_clock:1447124144348201014> **Set at:** {discord.utils.format_dt(current_time, 'F')}\n"
                           f"<:icons_clock:1447124144348201014> **Triggered after:** {amount} {time_units[unit]}\n"
            ...
            error_message = f"<:icons_Wrong:1447124112030826516> | An error occurred: {str(e)}"
    ...
    async def faq_category_select(self, interaction: discord.Interaction, select: discord.ui.Select):
        try:
            ...
            if category == "getting_started":
                embed = discord.Embed(
                    title="<:Ticket_icons:1447124189650878464> Getting Started with Support Tickets",
                ...
                embed.add_field(
                    name="<:UA_Rocket_icons:1447124128342478990> **How do I create a ticket?**",
                ...
                embed.add_field(
                    name="<:j_icons_Correct:1447124105311682673> **What happens after I create a ticket?**",
                ...
                embed.add_field(
                    name="<:lightbulb:1447124133107204166> **Pro Tips for New Users**",
                    value="• Be clear and detailed in your ticket description\n"
                          ...
                          "• Rate your experience when done! <:j_icons_Correct:1447124105311682673>",
                    inline=False
                )

            elif category == "response_priority":
                embed = discord.Embed(
                    title="<:icons_clock:1447124144348201014> Response Times & Priority Levels",
                ...
                embed.add_field(
                    name="<:icons_clock:1447124144348201014> **Expected Response Times**",
                ...
                embed.add_field(
                    name="<:Target:1447124315211567134> **How to Choose Priority**",
                ...
                embed = discord.Embed(
                    title="<:Target:1447124315211567134> Ticket Management Guide",
                ...
                embed.add_field(
                    name="<:j_icons_Correct:1447124105311682673> **Managing Your Tickets**",
                ...
                embed.add_field(
                    name="<:icons_clock:1447124144348201014> **Ticket Limits & Rules**",
                ...
                embed.add_field(
                    name="<:lightbulb:1447124133107204166> **Best Practices**",
                ...
                embed = discord.Embed(
                    title="<:clipboard1:1447124356130934835> Features & Settings",
                ...
                embed.add_field(
                    name="<:clipboard1:1447124356130934835> **Transcript System**",
                ...
                embed.add_field(
                    name="<:icons_star:1447124294545965109> **Rating System**",
                ...
                embed.add_field(
                    name="<:Target:1447124315211567134> **Advanced Features**",
                ...
                embed = discord.Embed(
                    title="<:icons_wrench:1447124178296766494> Troubleshooting Common Issues",
                ...
                embed.add_field(
                    name="<:icons_Wrong:1447124112030826516> **Common Problems & Solutions**",
                ...
                embed.add_field(
                    name="<:icons_wrench:1447124178296766494> **Quick Fixes**",
                ...
                embed.add_field(
                    name="<:UA_Rocket_icons:1447124128342478990> **Still need help?**",
                    value="Can't find what you're looking for?\n"
                          ...
                          "•  service guaranteed! <:j_icons_Correct:1447124105311682673>",
                    inline=False
                )
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    f"<:icons_Wrong:1447124112030826516> | An error occurred: {e}",
                    ephemeral=True
                )
            else:
