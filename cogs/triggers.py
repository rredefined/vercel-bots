...
class TriggerSystem(commands.Cog):
    ...
    async def add_trigger(self, ctx: commands.Context, keyword: str, *, message: str):
        ...
        try:
            ...
            if not self.triggers_db:
                error_msg = "<:icons_Wrong:1447124112030826516> | Database connection failed. Please try again later."
                if is_interaction:
            ...
            if len(keyword) > 50:
                error_msg = "<:icons_Wrong:1447124112030826516> | Keyword cannot exceed 50 characters."
            ...
            if len(message) > 2000:
                error_msg = "<:icons_Wrong:1447124112030826516> | Response message cannot exceed 2000 characters."
                if isinstance(ctx, discord.Interaction):
                    await ctx.followup.send(error_msg, ephemeral=True)
                else:
                    await ctx.send(error_msg)
            ...
            async with self.triggers_db.cursor() as cur:
                try:
                    ...
                    embed = discord.Embed(
                        title="<:j_icons_Correct:1447124105311682673> Trigger Added Successfully",
                    ...
                    error_msg = f"<:icons_Wrong:1447124112030826516> | A trigger for keyword `{keyword}` already exists in this server."
                    if isinstance(ctx, discord.Interaction):
                        await ctx.followup.send(error_msg, ephemeral=True)
                    else:
                        await ctx.send(error_msg)
    ...
    async def remove_trigger(self, ctx: commands.Context, keyword: str):
        ...
        try:
            ...
            async with self.triggers_db.cursor() as cur:
                ...
                if cur.rowcount > 0:
                    await self.triggers_db.commit()
                    success_msg = f"<:j_icons_Correct:1447124105311682673> | Trigger for keyword `{keyword}` has been removed."
                else:
                    success_msg = f"<:icons_Wrong:1447124112030826516> | No trigger found for keyword `{keyword}`."
    ...
    async def list_triggers(self, ctx: commands.Context):
        ...
        try:
            ...
            if not triggers:
                no_triggers_msg = "<:clipboard1:1447124356130934835> | No triggers configured for this server."
                if isinstance(ctx, discord.Interaction):
            ...
            embed = discord.Embed(
                title="<:clipboard1:1447124356130934835> Active Triggers",
            ...
            embed.add_field(
                name="<:Target:1447124315211567134> Keyword Triggers",
                value="\n".join(trigger_list),
                inline=False
            )
            ...
            if len(triggers) > 10:
                embed.add_field(
                    name="<:stats_1:1447124183686320259> Additional Info",
